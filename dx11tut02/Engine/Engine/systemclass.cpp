/////////////////////////////////////////////
//// This class sets up the system control //
/////////////////////////////////////////////
...
bool SystemClass::Frame()
{
	...
	// from start state/start screen, press enter to go to serve state
	if (m_Application->GetGameManager()->GetState() == States::Start)
	{
		if (m_Input->IsKeyDown(VK_RETURN))
			m_Application->GetGameManager()->ChangeState(States::Serve);
	}

	//from serve state, press space to start service and move to play state
	if (m_Application->GetGameManager()->GetState() == States::Serve)
	{
		if (m_Input->IsKeyDown(VK_SPACE))
			m_Application->GetGameManager()->ChangeState(States::Play);
	}

	//from done/game over state, press enter to restart the game
	if (m_Application->GetGameManager()->GetState() == States::Done)
	{
		if (m_Input->IsKeyDown(VK_RETURN))
			m_Application->GetGameManager()->ChangeState(States::Serve);
	}
	...
	//Check controls only if game is in play state
	if (m_Application->GetGameManager()->GetState() == States::Play)
	{

		// Input Controls for left paddle
		if (m_Input->IsKeyDown(0x57))
		{
			m_Application->GetPaddle(0)->Update(0.04f);
		}
		else
		{
			m_Application->GetPaddle(0)->Update(0.0f);
		}
		...
		if (m_Input->IsKeyDown(VK_UP))
		{
			m_Application->GetPaddle(1)->Update(0.04f);
		}
		else
		{
			m_Application->GetPaddle(1)->Update(0.0f);
		}
